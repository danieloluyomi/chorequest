...
app.get("/api/dashboard", async (req, res) => {
  ...
  const [profile] = await sql`SELECT u.timezone,COALESCE(p.theme,'garden') theme,COALESCE(p.animation,true) animation FROM users u LEFT JOIN preferences p ON p.user_id=u.id WHERE u.id=${current.id}`;
